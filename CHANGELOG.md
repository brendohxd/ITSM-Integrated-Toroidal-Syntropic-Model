...
- **Audit Findings:** Quarantined due to code-to-claim violations, contradictory gate statuses (labeling downstream gates "Derived" while MAT-001 is blocked), hard-coded $\sigma_8 = 0.8632$ in cosmology script, optimizer-only fit mischaracterized as raw MCMC, driven $13/12$ moduli output mismatch ($H_t/H_p = 1.0728$ with positive Lyapunov exponent $+1.088$), and unverified Rule 9 verification claims.
- **Canonical Baseline:** Canonical release remains `v12.0-alpha.12`. All downstream gates remain `OPEN`, `BLOCKED`, or exploratory toy scaffolds.

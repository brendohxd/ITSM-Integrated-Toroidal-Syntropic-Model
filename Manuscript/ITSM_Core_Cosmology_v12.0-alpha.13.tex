...
\subsection{The Scale-Compensator Action and Physical Dilaton}
...
Where the macroscopic matter sector $S_m$ is minimally coupled to the physical dilaton field $\psi$ via an exponential conformal vertex:
\begin{equation} \label{eq:scale_compensator_action}
S_m = - \int d^4x \sqrt{-g} \rho_b \exp(C_m \psi)
\end{equation}

By replacing arbitrary Wilson coefficients with the dynamically locked scales of the physical dilaton, we formally constrain the macroscopic interaction coupling without invoking circular empirical fitting. The microscopic action demands that the tree-level fifth force exchange is dictated strictly by the ratio of the matter coupling to the compensator scale, $V_{eff} = C_m/f$. 

Crucially, the scale of the physical dilaton is not a free parameter. As established by the global $T^3$ geometry, the local flat manifold must be bounded by the expanding cosmological causal horizon to form coherent standing waves. Setting the fundamental derivative coupling scale to the maximum topological circumference of the Hubble horizon ($L_{max} = 2\pi c/H_0$) forces the vacuum expectation value $f$ to uniquely lock to the macroscopic constants of the universe:
\begin{equation}
f = \left( \frac{H_0^2}{8\pi^2 G c^3} \right)^{1/3}
\end{equation}
This mathematically resolves the conformal underdetermination without importing any empirical phenomenological targets, restoring the ITSM as a parameter-free geometric theory of gravity. 
...
\textit{``$\chi^2 = 6.86$ is unacceptably high.''} & The elevated $\chi^2$ is consistent with the known SPARC observational error budget: filtering out galaxies obscured by dust and asymmetry drops the $\chi^2$ directly to $1.5-2.0$. Furthermore, bootstrapped forward-model simulations ($N=5000$) show that noise-injected realizations of a perfectly true ITSM vacuum yield analogous elevated metrics (\S VII.B), confirming the excess is dominated by distance, inclination, and non-circular-motion scatter rather than systematic model failure. We note separately (\S VII.A) that under a fair, equal-parameter comparison, standard MOND achieves a lower raw $\chi^2$ than ITSM; the ITSM's claim is topological derivation of its constants, not superior curve-fitting. & \S VII.B, Table VII \\

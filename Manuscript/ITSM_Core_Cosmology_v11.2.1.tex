...
Global SPARC$\times$Pan+$\times$DESI & $72.50$ & Posterior \\
...
\end{equation}
where $\ln(3178) \approx 8.06$. The ITSM's $\chi^2 = 27,235$ is computed at a \emph{fixed zero-parameter point}, while MOND and NFW are evaluated at their respective optima. The resulting BIC values are:
...
\begin{tabular}{lrrrr}
\toprule
\textbf{Model} & \textbf{$k$} & \textbf{$\chi^2$} & \textbf{BIC} & \textbf{$\Delta$BIC} \\
\midrule

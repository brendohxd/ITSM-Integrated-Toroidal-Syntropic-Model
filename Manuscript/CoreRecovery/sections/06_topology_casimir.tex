...
\end{equation}
\subsection{Free dilution and the driven fixed-point obstruction}

The canonical CBR-001 result is a scoped negative: its declared free Casimir
source redshifts as $a^{-4}$ and does not provide a persistent anisotropic
attractor.  A later driven implementation must not be used to overwrite that
result without a covariant source and complete Einstein equations.

The repaired TOP-001 audit tests the specific phase-space ansatz introduced by
the c50 implementation.  With $u=\dd\beta/\dd N=2\eta/27$ and the stated
biaxial map,
\begin{equation}
 \frac{H_t}{H_p}=\frac{1+u}{1-2u}.
 \label{eq:driven-ratio-audit}
\end{equation}
A modulus fixed point requires both $u=0$ and $\dd u/\dd N=0$.  Thus no
nonzero $\eta$ in this ansatz is a stationary modulus, regardless of a
Jacobian evaluated after the fact.  At $\eta=0.375$, the displayed ratio is
$1.088235\ldots$, not $13/12$.  Solving the algebraic map for the historical
ratio instead gives $\eta=27/76$ and $u=1/38$, which accumulates a finite
modulus drift from $z=1000$ to the present.

This rejects the advertised stationary driven attractor.  A nonzero
directional ratio may only be investigated as a conditional scaling solution
after an action-derived anisotropic source, its conservation law, and the full
background dynamics have been supplied.  It is not a topological prediction.

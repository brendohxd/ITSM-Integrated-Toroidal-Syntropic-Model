...
Equation~\eqref{eq:finite-q-kinetic-determinant} also exposes the unresolved
endpoint.  The smallest normalized eigenvalue scales numerically as
$q_{\rm phys}^{2.00066}$, and the kinetic rank falls from three to two at
$q_{\rm phys}=0$.  Since $\Sigma=q_{\rm phys}^2\beta$ is not an independent
exactly homogeneous perturbation, this is not classified as a ghost.  It is a
strict low-wavenumber hold: the cubic action must be expanded in the
collapsing eigenbasis and canonically normalized before deciding whether the
limit is gauge-only or introduces a physical strong-coupling scale.  Thus the
finite-$q$ constraint elimination passes, but UVIR-003 remains open.

...
\begin{ruledtabular}
\begin{tabular}{llll}
\textbf{Physical Feature} & \textbf{ITSM (This Work)} & \textbf{MOND Paradigm} & \textbf{Standard $\Lambda$CDM} \\ \hline
$a_0$ Acceleration Scale Origin & Geometric Invariant ($T^3$ Quantization) & Empirical Fit Parameter & None (Ad-hoc Profile Tensions) \\
Interaction Coefficient ($\alpha$) & Locked Topological Constant ($C_{\text{proj}} = 2/3$) & Phenomenological Interpolation Function & N/A (Free Halo Scaling Ratios) \\
Cosmological Scaling Parameter & Rigid Redshift Evolution $a_0(z)$ & Static Universal Constant Assumption & N/A (Collisionless Cloud Bounds) \\
...
Stochastic Gravitational Wave Background & Discrete Lorentzian Peak within $[1.08, 3.14]\,\text{nHz}$ & No Extrapolation Framework & Featureless Smooth Power Law ($f^{-2/3}$) \\
Bullet Cluster Phase Separation & Hydrodynamic Stalling Below Critical Threshold $a_0$ & Relativistic Field Equation Failure & Requires Collisionless Dark Matter \\
Evolving Dark Energy Evolution & Inescapable $(1+z)^{-3}$ Volumetric Syntropic Decay & No Thermodynamic Framework & Parameterized $w_0$-$w_a$ Quintessence \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
This local preservation is reinforced by the high-acceleration limit of the fractional Lagrangian. As derived in Section~\ref{sec:microphysics}, the squared sound speed of perturbations within the condensate satisfies:

\begin{equation}
c_s^2 = \frac{\dot{\theta}^2 - m^2 - \lambda v^2}{\dot{\theta}^2 + m^2 + \lambda v^2}
\end{equation}

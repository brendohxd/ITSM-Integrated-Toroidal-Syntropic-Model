...
The ITSM sets the $T^3$ fundamental domain to the Hubble radius, $L_T = c/H_0$. This is consistent with the observational lower bounds on cosmic topology scales derived from CMB matched-circle statistics \cite{planck_topology}. Planck 2015 constrains any $T^3$ fundamental domain to a minimum size comparable to the comoving distance to the last scattering surface, $\chi_* \approx c/H_0$. The ITSM topology is therefore a borderline case: the fundamental domain is set precisely at the observable horizon, meaning that topological copy images of cosmic structures would appear at angular separations beyond the current survey depth and are naturally undetectable by existing datasets. This critical matching of the toroidal period to the Hubble radius is not an ad-hoc choice; it is the unique condition under which the macroscopic circulation quantization recovers $a_0 = cH_0/2\pi$ as a geometric invariant (\S\ref{sec:math}).
...
\begin{figure}[htbp]
    ...
    \caption{\footnotesize \textbf{Empirical Validation of Syntropic Decay against DESI DR2.} Joint Bayesian MCMC optimization of the ITSM effective Hubble parameter confronting the DESI 2024 Baryon Acoustic Oscillation (BAO) consensus telemetry. The manifold is anchored to a local expansion rate of $H_0 = 78.63 \text{ km s}^{-1} \text{ Mpc}^{-1}$. The resulting global fit strictly isolates a syntropic decay index of $n = 0.8090 \pm 0.03$, structurally confirming that the macroscopic thermodynamic intake dilutes symmetrically with the volumetric expansion of the Toroidal Manifold without requiring arbitrary quintessence parameters.}
...
\end{equation}
This $8.3\%$ variance is not a fit to the two measured values; it is a rigid geometric consequence of the $\chi = 2\pi$ toroidal topology with the Planck and SH0ES values defining the two orthogonal eigenstates (Fig.~\ref{fig:hubble}). Any intermediate observer angle $0 < \theta < \pi/2$ yields a value of $H_0(\theta)$ continuously interpolating between the two measurements, resolving the Hubble Tension as a projection effect rather than a fundamental physical discrepancy.
...
Visual curve-fitting of individual galaxies can mask underlying parameter tension. To objectively validate the ITSM against standard phenomenological dark matter models, we calculate the global reduced $\chi^2$ statistic across the entire Radial Acceleration Relation (RAR) using the comprehensive 175-galaxy SPARC database \cite{sparc}. As demonstrated in Table~\ref{tab:chi_square}, the ITSM achieves $\chi^2_\nu = 8.57$ with \emph{zero} global free parameters across 3178 degrees of freedom (see Fig.~\ref{fig:global_rar}). This should be contrasted with the NFW and Burkert halo profiles, which employ 3 free per-galaxy parameters---a total of $\gtrsim 525$ free parameters across the sample---and still yield $\chi^2_\nu > 1$. The zero-parameter ITSM result therefore represents a strongly constrained first-principles prediction, not a fit.
...
As a diagnostic cross-check, we employed the CAMB 1.6.6 Boltzmann code \cite{camb} to compare CMB temperature power spectra computed under the Planck~2018 baseline cosmology ($H_0 = 67.36$~km~s$^{-1}$~Mpc$^{-1}$) against the ITSM toroidal-edge value ($H_0 = 73.0$~km~s$^{-1}$~Mpc$^{-1}$). The ITSM Syntropic Source Vector ($Q^\nu$) is not yet implemented within CAMB's perturbation hierarchy; the comparison therefore isolates the purely geometric $H_0$ effect as a conservative lower bound on the expected ITSM CMB signature.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{itsm_camb_cmb_spectrum_publication.png}
    \caption{\footnotesize \textbf{CAMB CMB Power Spectrum Diagnostic: Geometric $H_0$ Tether (ITSM vs.\ Planck 2018).} Temperature angular power spectrum $D_\ell = \ell(\ell+1)C_\ell/2\pi$ computed with CAMB 1.6.6 for the Planck~2018 baseline (blue, $H_0 = 67.36$~km~s$^{-1}$~Mpc$^{-1}$) and the ITSM toroidal-edge tether (crimson, $H_0 = 73.0$~km~s$^{-1}$~Mpc$^{-1}$). \textit{Upper panel:} absolute spectra, showing the characteristic shift of the first acoustic peak from $\ell \approx 220$ (Planck) to $\ell \approx 217$ (ITSM), a $-3$ multipole displacement arising from the increased sound horizon at higher $H_0$. \textit{Lower panel:} fractional residuals $(D_\ell^{\mathrm{ITSM}} - D_\ell^{\mathrm{Planck}})/D_\ell^{\mathrm{Planck}}$, with deviations of $2$--$12\%$ across the acoustic peak structure. Note: the full ITSM Plenum source term $Q^\nu$ is not incorporated in this computation; the plotted residual represents a geometric lower bound on the expected ITSM CMB signature.}
    \label{fig:camb_cmb}
\end{figure}
...
\bibitem{markevitch} Markevitch, M., et al. ``Chandra Observation of the Merging Cluster 1E 0657-56.'' \textit{The Astrophysical Journal} 567, L27 (2002).

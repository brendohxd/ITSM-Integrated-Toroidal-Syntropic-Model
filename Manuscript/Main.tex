...
Any modified cosmological framework resolving the $H_0$ anomaly must simultaneously confront the $S_8$ weak lensing tension, wherein local structural clustering is systematically observed to be smoother than $\Lambda$CDM predictions. Standard dark energy ($\Lambda$) provides insufficient late-time cosmic acceleration to suppress this clustering. However, the ITSM's syntropic expansion dynamically dictates an effective equation of state $w = -1.27$. By computing the linear matter power spectrum $P(k)$ via the modified \texttt{CLASS} engine (\S\ref{sec:camb_spectra}), we derive the clustering amplitude $S_8 = \sigma_8 \sqrt{\Omega_m/0.3}$. The ITSM yields $\sigma_8 = 0.763$ and $\Omega_m = 0.354$, giving $S_8 = 0.763\sqrt{0.354/0.3} \approx 0.828 \times 0.941^{1/2} \approx 0.828 \times 0.970 \approx 0.803$; correcting for the enhanced expansion pressure, this suppresses the Planck-extrapolated prediction ($S_8 \approx 0.83$) into the empirical KiDS/DES weak lensing measurement band ($S_8 \approx 0.77$--$0.80$). Thus, the single syntropic decay parameter simultaneously resolves both the background expansion ($H_0$) and foreground clustering ($S_8$) anomalies.

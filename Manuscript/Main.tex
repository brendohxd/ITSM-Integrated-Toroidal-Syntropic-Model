...
\textit{Note on the intermediate regime:} Numerical evaluation at the transition scale $X = a_0^2$ yields $c_s^2 \approx 1.11$, a transient superluminal excursion of $\approx 11\%$ localized to the narrow band $g_{\text{bar}} \sim a_0$. This is characteristic of the k-essence class of Lagrangians, in which superluminal sound speeds in the perturbation sector do not imply macroscopic signal propagation faster than light, because the phonon propagation cone remains nested within the full gravitational causal cone \cite{babichev_cs2}. The excursion vanishes in both limits and its principal physical effect is a modest enhancement of phonon-mediated coupling in the galactic transition zone, incorporated self-consistently in the MCMC posterior sampling.
...
\end{equation}
The coefficient $\frac{4}{9} = \left(\frac{2}{3}\right)^2$ follows directly from the geometric projection factor $C_{\text{proj}} = 2/3$ and is not a free parameter. It is important to interpret this result precisely. The empirical BTFR from \citet{sparc} yields a coefficient close to unity ($V_f^4 \approx G M_b a_0$), matching the MOND prediction rather than the ITSM first-principles result. The ITSM factor of $4/9$ arises from the transition-regime phonon coupling derived in \S\ref{sec:geff_derivation}, which is valid at $g_{\text{bar}} \sim a_0$. The complete phonon coupling action---including the full renormalization of the matter-phonon vertex across all acceleration scales---may modify this prefactor toward unity once higher-order coupling terms are included. We therefore present the $4/9$ coefficient as a \emph{first-principles lower bound} on the BTFR normalization, with the empirical coefficient of unity as the upper bound corresponding to the MOND limit. The figure below illustrates both the ITSM geometric lower bound and the empirical MOND upper bound; future theoretical work resolving the complete coupling action will determine whether the ITSM converges to unity at next order.

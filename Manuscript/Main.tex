...
\subsection{\label{sec:projection}Topological Anisotropy and the Hydrodynamic Derivation of the 2/3 Factor}

To definitively separate the Plenum Shear Ansatz from the empirical curve-fitting functions characteristic of Modified Newtonian Dynamics (MOND), we derive the $2/3$ interaction coefficient from the first principles of superfluid hydrodynamics constrained by a 3-Torus ($T^3$) topology. Within the ITSM framework, a localized baryonic mass density $M_b$ does not experience classical viscous drag; instead, it acts as a macroscopic, non-linear stirrer displacing the Superfluid Plenum condensate.

In a toroidal Bose-Einstein Condensate (BEC), the velocity field is strictly bound by the topological constraints of the manifold. The circulation around any closed, non-contractible phase-space loop is quantized according to the Onsager-Feynman constraint:

\begin{equation}
\oint \mathbf{v} \cdot d\mathbf{l} = n \kappa, \quad \kappa = \frac{h}{m}, \quad n \in \mathbb{Z}
\end{equation}

where $\kappa$ is the quantum of circulation. For a 3-Torus ($T^3$) manifold, the maximum causal phase space loop is bounded by the fundamental domain matching the Hubble radius $L = c/H_0$. This naturally isolates the minimum circulation quantum as $\kappa = c^2 / H_0$. As the baryonic stirrer moves through the plenum with kinetic energy $X = \frac{1}{2}M_b v^2$, it surpasses the Landau critical velocity, dissipating momentum via the continuous nucleation of quantized vortex rings. The energy of an isolated vortex ring scales linearly with its circumference, $E_{\text{vortex}} \propto \rho_s \kappa^2 R$, where $\rho_s = |\Phi|^2$ is the localized superfluid density and $R$ represents the characteristic radius of the baryonic obstacle.

The effective hydrodynamic drag force $F_{\text{drag}} = dE_{\text{vortex}}/dx \propto \rho_s \kappa^2$ exerted on the stirrer is intrinsically anisotropic due to the periodic boundary conditions of the $T^3$ manifold. The background spatial geometry is governed by the 3D metric $\gamma_{\mu\nu}$ with an immutable trace $\text{Tr}(\gamma_{\mu\nu}) = \gamma^\mu_\mu = 3$. However, the nucleation of the 2D vortex loops is strictly confined to the transverse shear plane orthogonal to the stirrer's trajectory vector $n^\mu$, defined by the induced metric $h_{\mu\nu} = \gamma_{\mu\nu} - n_\mu n_\nu$ with a trace $\text{Tr}(h_{\mu\nu}) = h^\mu_\mu = 2$.

The geometric projection of the 3D stirrer volume onto the 2D fluid shear surface dictates the momentum transfer efficiency. This topological restriction behaves as the fluid-dynamic analogue of the demagnetization tensor in classical electrodynamics. In a symmetric 3D system, the depolarization factors sum to unity ($\sum N_i = 1$), splitting into eigenvalues of $1/3$ per unconstrained dimension. Because the periodic boundary conditions of the $T^3$ geometry suppress dissipation parallel to the flow, the effective drag is dictated strictly by the two transverse axes of the induced metric plane. The resulting structural projection factor is locked by the ratio of the respective metric traces:

\begin{equation}
C_{\text{proj}} = \frac{\text{Tr}(h_{\mu\nu})}{\text{Tr}(\gamma_{\mu\nu})} = \frac{2}{3}
\end{equation}

This formal derivation mathematically locks the $2/3$ coefficient in the Plenum Shear Ansatz. It proves that the scaling factor is an un-tuned, rigid consequence of topological embedding and Gross-Pitaevskii vortex mechanics, leaving the model with zero phenomenological degrees of freedom.
...
\subsection{\label{sec:ppn}Explicit Parameterized Post-Newtonian (PPN) Parameterization}

To satisfy the strict parameterized post-Newtonian (PPN) limits without breaking cosmic-scale modifications, the Einstein Field Equations must be explicitly formalized to include the Syntropic Source Vector $Q_\mu$. We express the comprehensive modified field equations as:

\begin{equation}
G_{\mu\nu} = \frac{8\pi G}{c^4} \left( T_{\mu\nu}^{(m)} + T_{\mu\nu}^{(P)} \right)
\end{equation}

where $T_{\mu\nu}^{(m)}$ is the baryonic stress-energy tensor and $T_{\mu\nu}^{(P)}$ is the Superfluid Plenum energy tensor. Bianchi compliance requires $\nabla_\mu G^{\mu\nu} = 0$, which is satisfied via the cross-sector coupling:

\begin{equation}
\nabla_\mu T^{\mu\nu}_{(m)} = Q^\nu, \quad \nabla_\mu T^{\mu\nu}_{(P)} = -Q^\nu
\end{equation}

We execute the PPN metric expansion around the flat Minkowski background $\eta_{\mu\nu}$ in the local weak-field, high-acceleration limit ($g_N \gg a_0$):

\begin{align}
g_{00} &= -1 + 2\phi - 2\beta \phi^2 + \mathcal{O}(\phi^3) \\
g_{ij} &= \delta_{ij} (1 + 2\gamma \phi) + \mathcal{O}(\phi^2)
\end{align}

where $\phi = G M_b / r c^2$ is the standard Newtonian potential.

The Plenum Shear Ansatz contains a non-linear Born-Infeld saturation root. In the inner Solar System, where the localized acceleration scale is violently high compared to the cosmic threshold ($g_N \approx 10^{-2}\,\text{m/s}^2 \gg a_0 \sim 10^{-10}\,\text{m/s}^2$), the saturation function forces the interaction cross-section of the plenum tensor to decay as:

\begin{equation}
T_{\mu\nu}^{(P)} \propto \mathcal{O}\left(\frac{a_0^2}{g_N^2}\right) \implies Q^\nu \to 0
\end{equation}

Evaluating the metric coefficients under these highly localized boundary conditions yields the analytic values:

\begin{equation}
\gamma = 1.00000 \pm \mathcal{O}(10^{-16}), \quad \beta = 1.00000 \pm \mathcal{O}(10^{-16})
\end{equation}

The derived values for the space-curvature parameter $\gamma$ and the non-linearity parameter $\beta$ fall deep within the experimental limits established by the Cassini spacecraft radio-link measurement ($\gamma - 1 = (2.1 \pm 2.3) \times 10^{-5}$) \cite{cassini}. The ITSM smoothly collapses to standard General Relativity in high-acceleration zones, ensuring light bending and Shapiro time delays match local observations perfectly while isolating non-Newtonian effects to low-acceleration galactic and cosmic regimes.
...
\end{align}
To ensure computational falsifiability, we must explicitly define the acoustic shear source term $f_{\text{ITSM}}(a_0, \delta)$. The functional form is derived by linearizing the Plenum Shear Ansatz in the low-acceleration regime ($k^2\Psi/a \ll a_0$) and coupling it directly to the volumetric expansion of the Toroidal Manifold:

\begin{equation}
f_{\text{ITSM}}(a_0, \delta) = \frac{2}{3} \mathcal{H} \left( \frac{c_s^2 k^2}{a^2 a_0} \right) \delta_m \Xi(z)
\end{equation}

where $\mathcal{H}$ is the conformal Hubble parameter, $c_s^2$ is the fluid sound speed, and $\Xi(z)$ is the critical thermodynamic state function that governs the phase transition of the Superfluid Plenum. 

A paramount constraint in non-standard cosmology is preserving the pristine acoustic peaks of the Cosmic Microwave Background (CMB). During the radiation-dominated era ($z \gg 3000$), the universe operates as a tightly coupled, high-temperature baryon-photon plasma. The macroscopic vacuum lacks the thermodynamic stability to sustain a cohesive Bose-Einstein condensate. Therefore, the state function must mathematically enforce a decoupling phase:

\begin{equation}
\Xi(z) = \left[ 1 + e^{\lambda(z - z_c)} \right]^{-1}
\end{equation}

where $z_c \approx 3400$ represents the critical condensation redshift (equivalent to the epoch of matter-radiation equality) and $\lambda$ dictates the rapidity of the phase transition. 

At early times ($z \gg z_c$), the state function rigorously zeroes out ($\Xi(z) \to 0$), completely deactivating the acoustic shear source term. The perturbation equations perfectly collapse back to standard General Relativity, guaranteeing that the ITSM preserves the acoustic plasma oscillations constrained by Planck 2018. As the universe cools and transitions into matter domination ($z \ll z_c$), the macroscopic condensate crystallizes, the state function saturates ($\Xi(z) \to 1$), and the fractional vacuum drag smoothly engages to drive the accelerated linear growth rate of Large-Scale Structure.

\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.85\textwidth]{itsm_thermodynamic_decoupling_publication.png}
    \caption{\footnotesize Superfluid Plenum Phase Transition and CMB Acoustic Protection. The ITSM thermodynamic state function $\Xi(z)$ mathematically zeroes out the fractional vacuum drag during the radiation-dominated era ($z > 3400$), completely shielding the pristine baryon-photon plasma from acoustic shear. As the universe expands and cools into the matter-dominated epoch, the Superfluid Condensate crystallizes ($\Xi(z) \to 1$), smoothly engaging the macroscopic torsional drag required to accelerate large-scale structure formation.}
    \label{fig:decoupling}
\end{figure}

\subsection{\label{sec:quench}Localized Vacuum Quenching and Extreme-Energy Falsifiability}

A foundational tenet of the ITSM is the formal classification of the macroscopic vacuum as a Superfluid Plenum—an effective Bose-Einstein Condensate. Consequently, the vacuum must strictly adhere to the thermodynamic boundary limits governing condensed matter physics. In applied superconductivity and superfluidity, localized thermal or kinetic energy injection exceeding the critical state boundary induces a localized phase transition known as a ``quench,'' wherein the condensate abruptly reverts to a normal, high-friction fluid state. 

Extrapolating this universal thermodynamic principle to the cosmological manifold, the ITSM provides a highly specific, observationally testable boundary limit. Standard $\Lambda$CDM models treat the dark matter halo as a cloud of collisionless particles whose kinematics are invariant to localized baryonic thermal extremes. Conversely, the ITSM dictates that within spatial regimes of extreme baryonic energy injection—such as the relativistic jets of Active Galactic Nuclei (AGN) or the high-Mach thermal shockwaves of merging galaxy clusters (e.g., 1E 0657-56, the ``Bullet Cluster'')—the Superfluid Plenum must undergo a localized quench.

Within these extreme thermal boundary layers, the localized kinetic energy violently overrides the macroscopic state function ($\Xi(z) \to 0$ locally), temporarily severing the acoustic shear source term ($f_{\text{ITSM}} \approx 0$). Therefore, the ITSM explicitly predicts a severe, anomalous deficit of apparent kinematic ``dark matter'' effects restricted entirely to the immediate spatial vicinity of ultra-high-energy baryonic events. 

This provides a definitive avenue for observational falsifiability: if next-generation kinematic telemetry demonstrates that standard collisionless dark matter profiles are perfectly, uniformly preserved deep within these extreme, localized thermal quenching zones, the macroscopic condensate premise of the ITSM is strictly falsified.
...
\bibitem{sparc_tokamak_paper} Creely, A. J., et al. ``Overview of the SPARC tokamak.'' \textit{Journal of Plasma Physics} 86, 865860502 (2020).
\bibitem{desi} Adame, A. G., et al. (DESI Collaboration). ``DESI 2024 VI: Cosmological Constraints from the Measurements of Baryon Acoustic Oscillations.'' \textit{arXiv preprint arXiv:2404.03002} (2024).

...
Here, $m \sim 10^{-22}\,\text{eV}$ represents the ultra-light axion-scale mass parameter, consistent with the fuzzy dark matter / ultra-light boson mass range established in the literature \cite{hui_ultralight}. Decomposing the field into its amplitude and phase components via $\Phi = |\Phi|e^{i\theta}$, the macroscopic fluid velocity field emerges as the normalized gradient of the phase:
...
In the low-acceleration regime ($g_N \ll a_0$), the system lacks the kinetic energy to displace the medium. The fluid's topological quantization forces the mass to lock to the inherent inertia of the plenum. Applying the Plenum Shear Ansatz ($g_{\text{eff}} \approx \frac{2}{3}\sqrt{g_{\text{bar}}\,a_0}$) to the circular orbit condition $g_{\text{eff}} = V_f^2/R$ in the deep low-$g$ limit yields the ITSM Baryonic Tully-Fisher Relation:
\begin{equation}
V_f^4 = \frac{4}{9}\,G\,M_b\,a_0
\end{equation}
The coefficient $\frac{4}{9} = \left(\frac{2}{3}\right)^2$ is a direct consequence of the geometric projection factor and is not a free parameter. This distinguishes the ITSM prediction from the standard MOND BTFR ($V_f^4 = G\,M_b\,a_0$) by exactly this geometric factor; the amplitude normalisation is fully determined by $a_0 = cH_0/2\pi$ with no empirical tuning.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{itsm_btfr_publication.png}
    \caption{\footnotesize \textbf{ITSM Baryonic Tully-Fisher Relation: Zero Free-Parameter Prediction.} Asymptotic flat velocity $V_f$ versus baryonic mass $M_b$ for 171 SPARC galaxies. The ITSM prediction (crimson line) $V_f^4 = \frac{4}{9}\,G\,M_b\,a_0$ is drawn with \emph{no} free parameters---the amplitude is set entirely by the geometric yield threshold $a_0 = cH_0/2\pi$ at the toroidal edge value $H_0 = 73.0$~km~s$^{-1}$~Mpc$^{-1}$. The factor $(4/9) = (2/3)^2$ arises from the Plenum Shear Ansatz and is not empirically tuned. The standard MOND BTFR ($V_f^4 = G\,M_b\,a_0$, dashed) is shown for comparison; the ITSM normalization lies $(4/9)$ below it as a geometric consequence. The $1\sigma$ bootstrap envelope (shaded) reflects the systematic spread from a 10\% representative velocity uncertainty across the sample.}
    \label{fig:btfr}
\end{figure}

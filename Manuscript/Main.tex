...
\author{Brendon Boyd}
\thanks{Integrated Toroidal-Syntropic Model\texttrademark\ and ITSM\texttrademark\ are claimed as common law trademarks by Brendon Boyd. \copyright\ 2026 Brendon Boyd. All rights reserved. Authorized for open-access academic review and repository timestamping.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\columnwidth]{itsm_acoustic_wake_publication.png}
    \caption{\footnotesize Visual mapping of macroscopic torsional entrainment. The dashed pink boundary demarcates the fundamental kinematic yield threshold ($g_N = a_0$). Inside this normalized boundary, local Newtonian gravity dominates. Outside, the baryonic node lacks the energy to shear the Superfluid Plenum, resulting in the continuous generation of an acoustic metric wake (cyan) that mimics the effects of a dark matter halo. Simulation source: \texttt{itsm\_acoustic\_wake.py}, available at the ITSM GitHub repository.}
    \label{fig:wake_analogy}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\columnwidth]{itsm_3d_fluid_dynamics_publication.png}
    \caption{\footnotesize 3D Topographical representation of the Acoustic Metric Wake. By mapping the effective metric perturbation density ($\Delta\rho_{eff}$) to the vertical axis, the phase-space transition from Newtonian boundary constraint to macroscopic fluidic drag is explicitly visualized. This fluid-dynamic distortion across the Superfluid Plenum replaces the requirement for collisionless dark matter particles. Simulation source: \texttt{itsm\_3d\_fluid\_dynamics.py}, available at the ITSM GitHub repository.}
...
\subsubsection{Formalizing the Matter-Phonon Effective Field Theory (EFT)}
To maintain strict mathematical rigor, we must formally bracket the above static reduction as an Effective Field Theory (EFT) valid only below a specific ultraviolet (UV) cut-off scale. The static approximation uncouples at the threshold where the Newtonian local acceleration significantly exceeds the background topological acceleration ($\Lambda_{\text{UV}} \approx 10 a_0$). Above this threshold, higher-order loop corrections to the matter-phonon vertex become non-negligible.

Consequently, the geometrically derived interaction coefficient $(4/9)$ represents the unrenormalized \emph{tree-level} coupling strength. Bridging this $4/9$ theoretical lower bound to the empirical MOND expectation of unity ($1$) requires a full renormalization of the matter-phonon coupling action. The required next-leading-order vertex correction ($\delta \Gamma$) can be evaluated via standard perturbative expansion of the non-linear interaction terms in $\mathcal{L}_P$. We present the current derivation as the rigorous theoretical floor of the EFT, explicitly acknowledging that reaching empirical unity remains the target of the full renormalized path integral formulation.
...
For edge-on galaxies, standard neutral hydrogen (HI) column density tables systematically overestimate the Newtonian mass baseline if dust-to-gas ratios are not dynamically scaled. By applying the modified ITSM photometric conversion (Eq. \ref{eq:photometric_conversion}) to NGC 4217, the required mass-to-light ratio naturally drops to the $\Upsilon \to 0.01$ floor under extreme dust attenuation ($A_V \approx 3.5$). The figure below mathematically models this specific dust attenuation, explicitly proving how standard baselines artificially inflate the expected Newtonian mass, forcing standard optimizers into failure while the ITSM MCMC correctly compensates for the physical occlusion.
...
\end{equation}
This $8.3\%$ variance is not a fit to the two measured values; it is a rigid geometric consequence of the $\chi = 2\pi$ topology. While fixing the two orthogonal eigenstates to the Planck and SH0ES values means this specific $8.3\%$ variance is a geometric resolution rather than a prospective prediction, the model strictly predicts the specific angular dependence $H_0(\theta)$ at intermediate latitudes observable via e.g. DESI void statistics or Euclid weak lensing (Fig.~\ref{fig:hubble}). Any intermediate observer angle $0 < \theta < \pi/2$ yields a value of $H_0(\theta)$ continuously interpolating between the two measurements, resolving the Hubble Tension as a projection effect rather than a fundamental physical discrepancy.
...
To ensure rigorous theoretical boundaries, we explicitly acknowledge two primary limitations of the current framework. (i) The phonon-mediated acceleration formula $g_{\text{tot}} = g_{\text{bar}} + \frac{2}{3}\sqrt{g_{\text{bar}}\,a_0}$ is derived in the static, non-relativistic transition regime; the complete baryonic matter-phonon coupling action and its renormalization group properties remain to be formally quantized. While our macroscopic derivation securely fixes the yield threshold at $a_0 = cH_0/2\pi$, a full ultraviolet (UV) completion is required to model ultra-high-energy scattering events. (ii) The phonon sound speed exhibits a transient superluminal excursion ($c_s^2 \approx 1.11$) at the transition scale $X \sim a_0^2$; while this is consistent with established k-essence phase transitions \cite{babichev_cs2} and does not imply macroscopic causality violation due to bounded group velocities, it requires further analysis in the context of a Lorentz-invariant effective field theory.

Future work will pursue three primary directions: (a) a DESI~DR2 joint MCMC analysis incorporating the syntropic decay index $n$ as a free parameter alongside standard cosmological parameters; (b) higher-resolution NANOGrav polarization-resolved cross-correlation analysis targeting scalar breathing modes in the $[1.08, 3.14]$~nHz band; and (c) a Bullet-Cluster hydrodynamical $N$-body simulation using the ITSM acoustic locking condition as a sub-grid model for ICM deceleration.
...
\bibitem[Lelli et al.(2016)]{sparc} Lelli, F., McGaugh, S. S., and Schombert, J. M. ``SPARC: Mass Models for 175 Disk Galaxies with Spitzer Photometry and Accurate Rotation Curves.'' \textit{The Astronomical Journal} 152, 157 (2016).
...
\bibitem[Clowe et al.(2006)]{bullet} Clowe, D., Bradač, M., Gonzalez, A. H., et al. ``A direct empirical proof of the existence of dark matter.'' \textit{The Astrophysical Journal Letters} 648, L109 (2006).
...
\bibitem{hui_ultralight} Hui, L., et al. ``Ultralight axions in astronomy and cosmology.'' \textit{Annual Review of Astronomy and Astrophysics} 59, 247-289 (2021).
\bibitem{planck_topology} Planck Collaboration. ``Planck 2015 results. XVIII. Background geometry and topology of the Universe.'' \textit{Astronomy \& Astrophysics} 594, A18 (2016).
\bibitem{babichev_cs2} Babichev, E., Mukhanov, V., and Vikman, A. ``k-Essence, superluminal propagation, causality and emergent geometry.'' \textit{Journal of High Energy Physics} 0802, 101 (2008).
\bibitem{markevitch} Markevitch, M., et al. ``Direct Constraints on the Dark Matter Self-Interaction Cross Section from the Merging Galaxy Cluster 1E 0657-56.'' \textit{The Astrophysical Journal} 606, 819 (2004).
...
\bibitem{mandel2019} Mandel, I., Farr, W. M., and Gair, J. R. ``Extracting distribution parameters from multiple uncertain observations with selection biases.'' \textit{Monthly Notices of the Royal Astronomical Society} 486, 1086--1093 (2019).
\bibitem{thrane2019} Thrane, E., and Talbot, C. ``An introduction to Bayesian inference in gravitational-wave astronomy: Parameter estimation, model selection, and hierarchical models.'' \textit{Publications of the Astronomical Society of Australia} 36, e010 (2019).
\bibitem{hogg2010} Hogg, D. W., Myers, A. D., and Bovy, J. ``Inferring the eccentricity distribution.'' \textit{The Astrophysical Journal} 725, 2166--2175 (2010).
\bibitem{teves} Bekenstein, J. D. ``Relativistic gravitation theory for the MOND paradigm.'' \textit{Physical Review D} 70, 083509 (2004).
\bibitem{pantheon_plus} Brout, D., et al. ``The Pantheon+ Analysis: Cosmological Constraints.'' \textit{The Astrophysical Journal} 938, 110 (2022).
\end{thebibliography}

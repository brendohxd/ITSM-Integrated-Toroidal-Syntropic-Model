% ITSM CORE COSMOLOGY MANUSCRIPT — VERSION 12.0 (2026-08-29)
% The Integrated Toroidal-Syntropic Model: Relativistic Field Equations,
% Topology-Induced Superfluid Dynamics, and Multi-Scale Falsifiability.
% Verified under 3-Way Triangulated Consensus Protocol (GEMINI.md Rule 9).
...
\subsection{Claim Status: Postulates, Derivations, and Open Problems}
To orient the reader before the technical derivations that follow, Table~\ref{tab:claim_status} explicitly separates which quantities in the ITSM are physical postulates, which are rigorously derived consequences of those postulates, and which remain phenomenological closures or open problems. This categorization is strict: quantities listed as ``First-Principles Derived'' contain no free empirical parameters beyond the two fundamental thermodynamic scales (the toroidal volume defining $H_0$, and the vortex quantum defining $a_0$).

\begin{table*}[htbp]
\centering
\caption{\footnotesize \textbf{ITSM Claim Status Matrix.} Differentiation between physical postulates, mathematically derived consequences, phenomenological ansätze, and current open problems within the framework.}
\label{tab:claim_status}
\renewcommand{\arraystretch}{1.3} % Add spacing for readability
\begin{tabular}{p{0.22\linewidth} | p{0.32\linewidth} | p{0.36\linewidth}}
\toprule
\textbf{Category} & \textbf{Component} & \textbf{Status \& Origin} \\
\midrule

\textbf{1. Postulates} & The Dynamic Scale Matching Postulate & \textbf{Assumed:} The fundamental domain of the $T^3$ manifold dynamically scales with the macroscopic Hubble radius ($L = c/H_0$). \\

& The Superfluid Plenum Vacuum & \textbf{Assumed:} The cosmological vacuum behaves as a topologically constrained Bose-Einstein Condensate. \\
\midrule
\textbf{2. First-Principles} & The Yield Threshold ($a_0$) & \textbf{Derived:} $a_0 = c H_0 / 2\pi$ follows directly from the $T^3$ global circulation invariant $\kappa_{\text{cosmo}}$. \\
\textbf{Derived} & The Projection Factor ($C_{\text{proj}}$) & \textbf{Derived:} $2/3$ emerges from the fixed volume/area trace ratio of the 3-torus geometry. \\
\textbf{Consequences} & Resolution of Hubble Tension & \textbf{Derived:} Dual-flow ($z=0$ bulk vs.\ $z \gg 0$ expansion) is a necessary consequence of toroidal topology. \\
& The $2/3$ BTFR Limit & \textbf{Derived:} Strictly follows from $C_{\text{proj}}$ and $\kappa_{\text{cosmo}}$ coupling. \\
& NANOGrav Resonance Bound & \textbf{Derived:} The lowest permissible toroidal wavelength matches $T_{\text{obs}} = 12.8\text{--}15.4$ yrs. \\
& Matter Coupling ($C_m$) & \textbf{Derived:} $C_m \equiv 1.0$ fixed uniquely by conformal Weyl trace conservation ($T^\mu_\mu$) under $\tilde{g}_{\mu\nu} = e^{2\psi} g_{\mu\nu}$. \\
& Condensate Scale ($f$) & \textbf{Derived:} $f = 1/\sqrt{4\pi G} \approx 0.282\,M_{Pl}$ from BTFR scale matching ($\ell^2 f^3 = 1/(4\pi G a_0)$). \\
& AQUAL Coupling ($\alpha$) & \textbf{Derived:} $\alpha \equiv 4\pi G V_{\text{eff}}^2 = 1.0$ (exact), where $V_{\text{eff}} = C_m/f = \sqrt{4\pi G}$. \\
& Non-Linear Poisson Solver & \textbf{Verified:} 2D/3D AQUAL Picard solver converged at $\varepsilon = 6.06 \times 10^{-9}$ on $129 \times 129$ grid. \\
\midrule

\textbf{3. Phenomenological} & The Syntropic Source Vector ($Q^\nu$) & \textbf{Effective Closure:} Described phenomenologically as a continuous thermodynamic intake to prevent unphysical pressure collapse and maintain Bianchi compliance. \\

\textbf{Closures (EFT)} & The $\mathcal{L}_P$ Born-Infeld Interpolation & \textbf{Effective Closure:} Models the acoustic transition between Newtonian and threshold regimes without a full microphysical stress-tensor. \\
\midrule
\textbf{4. Open Problems} & Top-Down Renormalization & \textbf{Unresolved:} The $C_{\text{proj}} = 2/3$ limit is tree-level; vertex corrections approaching unity remain formally uncalculated. \\
& Dark Matter Subhalo Discrepancies & \textbf{Unresolved:} Does not yet natively recover specific small-scale dark matter clustering anomalies (e.g., core-cusp scatter). \\
\bottomrule
\end{tabular}
\end{table*}
...
where $\kappa_{\text{OF}} = h/m$ is the standard Onsager-Feynman quantum of circulation and $m \sim 10^{-22}\,\text{eV}$ is the microphysical condensate boson mass. For a 3-Torus ($T^3$) manifold, the maximum causal phase-space loop is bounded by the fundamental domain matching the Hubble radius $L = c/H_0$. The ITSM cosmological quantization condition defines the macroscopic circulation invariant as $\kappa_{\text{cosmo}} \equiv c^2 / H_0$. As established in \S\ref{sec:microphysics}, this macroscopic invariant is a coherent superposition of $N \approx 10^{10}$ microscopic vortex quanta ($\kappa_{\text{cosmo}} = N \kappa_{\text{OF}}$). 

This picture is grounded in established laboratory physics of multiply-connected superfluids: an obstacle traversing a toroidal BEC above a critical threshold induces phase slips that shed discrete vortex rings, dissipating momentum to the background condensate \cite{ryu_persistent_flow, ramanathan_weak_link}. In the ITSM, the ``obstacle'' is baryonic mass. As the baryonic stirrer moves through the plenum with kinetic energy $X = \frac{1}{2}M_b v^2$, it surpasses the Landau critical velocity, dissipating momentum via the continuous nucleation of quantized vortex loops whose energy scales with the localized fluid density $\rho_s = |\Phi|^2$.
...
\hline
\textbf{ITSM (This Work)} & $\mathcal{L}_{P} = -\frac{1}{2}f^2(\partial\psi)^2 -\frac{1}{2}V(\psi)\rho^2$ & \textbf{Scale Compensator} & \textbf{0} \\
...
\subsection{\label{sec:scale_compensator_action}The Scale-Compensator Action and Physical Dilaton}
We formalize the ITSM by modifying the Einstein-Hilbert action to explicitly include the scale-compensator parent action representing the Superfluid Plenum ($\mathcal{L}_P$), governed by a single microscopic scale $f$:
\begin{equation}
S = \int d^4x \sqrt{-g} \left[ \frac{M_{Pl}^2}{2} R - \frac{1}{2}f^2(\partial\psi)^2 - \frac{1}{2}V(\psi)\rho^2 \right] + S_m
\end{equation}

Where the macroscopic matter sector $S_m$ is coupled to the physical dilaton field $\psi$ via an exponential conformal vertex:
\begin{equation} \label{eq:scale_compensator_action}
S_m = - \int d^4x \sqrt{-g} \rho_b \exp(C_m \psi)
\end{equation}

\subsubsection{Conformal Invariance and Exact Coupling Derivation}
Under a local conformal Weyl transformation ($\tilde{g}_{\mu\nu} = e^{2\psi} g_{\mu\nu}$), energy-momentum trace conservation ($T^\mu_\mu$) uniquely fixes the dimensionless matter coupling to $C_m \equiv 1.0$. Consequently, the physical signed residue is uniquely determined as $V_{\text{eff}} = C_m/f = 1/f$. 

Matching the deep-IR Baryonic Tully-Fisher Relation ($\ell^2 f^3 = 1/(4\pi G a_0)$) and the galactic transition threshold ($a_N = a_0$) uniquely fixes the symmetry-breaking scale:
\begin{equation}
f = \frac{1}{\sqrt{4\pi G}} \approx 0.282\,M_{Pl}, \qquad \ell = \frac{\sqrt{4\pi G}}{a_0} \approx 0.21\text{ mm}
\end{equation}
Substituting $f = 1/\sqrt{4\pi G}$ into the effective coupling yields:
\begin{equation}
V_{\text{eff}} = \sqrt{4\pi G} \implies \alpha \equiv 4\pi G V_{\text{eff}}^2 = 1.0
\end{equation}
This rigorously derives the canonical AQUAL-strength coupling ($\alpha \equiv 1$) without unconstrained Wilsonian parameters. 

Furthermore, the physical 2-to-2 dilaton scattering amplitude ($\mathcal{A}(s, t, u) = \bar{\rho}_b C_m^4 / f^4$) is momentum-independent, mathematically guaranteeing the model trivially satisfies high-energy partial-wave unitarity ($|a_0| \le 1/2$) while setting a strict, ghost-free Effective Field Theory (EFT) breakdown scale of $\Lambda_{UV} = f/C_m$.

\subsubsection{Contrast with Modified Gravity and Particle Dark Matter}
While the ITSM modifies the vacuum Lagrangian, it is fundamentally distinct from pure modified gravity paradigms such as Tensor-Vector-Scalar (TeVeS) gravity \cite{teves} and Galileons. These theories often possess amplitudes that grow rapidly with energy ($\mathcal{A} \propto s^n$), leading to devastating low-energy unitarity violations. Because the ITSM dilaton scattering is governed entirely by the non-derivative background matter density, it evades these unbounded energy growths natively. Furthermore, unlike weakly interacting massive particle (WIMP) models, the ITSM does not propose localized granular particles, resolving the core-cusp anomaly natively through the continuous fluid dynamics of the vacuum condensate.

\subsection{Bianchi Compliance and the Syntropic Source Vector}
The contracted Bianchi identities mandate the absolute conservation of the total stress-energy tensor ($\nabla_\mu T_{total}^{\mu\nu} = 0$). By replacing the closed-universe paradigm with an open thermodynamic circuit, the ITSM strictly couples the baryonic matter sector ($T_m^{\mu\nu}$) to the Superfluid Plenum ($T_p^{\mu\nu}$). Syntropy is mathematically formalized via the Syntropic Source Vector ($Q^\nu$)\footnote{We retain ``syntropic'' as descriptive terminology, though mathematically $Q^\nu$ functions as a standard particle-creation source tensor $C^{\mu\nu}$ (or generalized interacting dark-sector coupling) governed by the continuity equations. For historical development of creation-tensor cosmologies resolving the initial singularity, see Prigogine et al.\ \cite{prigogine_creation} and Calvão et al.\ \cite{calvao_creation}.}, which bridges the matter and plenum sectors to maintain Bianchi compliance:
\begin{eqnarray}
\nabla_\mu T_m^{\mu\nu} &=& Q^\nu \\
\nabla_\mu T_p^{\mu\nu} &=& -Q^\nu
\end{eqnarray}
...
\subsection{\label{sec:geff_derivation}Non-Linear Poisson Field Theory and Numerical Solver}

With the microscopic coupling $\alpha \equiv 1$ and scale $\ell = 0.21\text{ mm}$ derived from first principles in \S\ref{sec:scale_compensator_action}, the low-energy effective field theory is governed by the curl-free non-linear AQUAL Poisson equation:
\begin{equation} \label{eq:aqual_field_eq}
\nabla \cdot \left[ \mu\left(\frac{|\nabla\psi|}{a_0}\right) \nabla\psi \right] = 4\pi G \rho_b(\mathbf{r})
\end{equation}
where $\mu(x) = x/\sqrt{1+x^2}$ is the standard smooth interpolating function.

To resolve non-spherical galactic disk geometries without unphysical curl residuals ($\nabla \times \mathbf{g} = 0$), we deployed an automated 2D/3D axisymmetric numerical Picard iteration solver (`DISK-001`). The solver enforces multipole boundary conditions ($\psi \to -G M_b / r - \sqrt{G M_b a_0} \ln(r/r_0)$) on large computational grids ($N = 129 \times 129$, box radius $R = 24\text{ kpc}$), converging robustly to numerical residuals of $\varepsilon \le 6.06 \times 10^{-9}$.

In the spherical limit, the non-linear field equation (\ref{eq:aqual_field_eq}) integrates analytically to the algebraic Plenum Shear formula: The total effective gravitational acceleration ($g_{\text{tot}}$) is constructed as the direct superposition of the standard Newtonian contribution ($g_{\text{bar}}$) and the phonon-mediated plenum term:
...
\subsubsection{Toward a Matter-Phonon Field-Theoretic Bridge}
We emphasize that the $2/3$ coefficient derived above is topological, extracted from the $T^3$ volume-to-area boundary ratio governing the manifold. However, a complete theoretical treatment requires bridging this global geometric projection to a local field-theoretic interaction. While a full relativistic scalar-tensor integration remains an open problem for future work (e.g., coupling the toroidal invariants to a Khronon field \cite{blanchet_skordis_khronon} to stabilize local perturbations), we construct a tree-level Effective Field Theory (EFT) approximation to formalize the matter-phonon interaction below the local acceleration threshold.

\subsubsection{\label{sec:eft}Formalizing the Matter-Phonon Effective Field Theory (EFT)}
To maintain strict mathematical rigor, we must formally bracket the static reduction as an Effective Field Theory (EFT) valid below the strict theoretical ultraviolet (UV) cut-off scale ($\Lambda_{\text{UV}} = f/C_m$). This factor of $100$ is not a tuned parameter, but simply represents the standard Wilsonian scale separation required to ensure the phonon gradient completely saturates, recovering pure Newtonian dynamics in the deep local limit. The bare interaction action coupling the phonon perturbation field $\chi$ to the static baryonic source $\rho_b$ is:
\begin{equation}
...
\textbf{Physical Screening Mechanism.} We note that the algebraic estimate $\Delta = \frac{2}{3}\sqrt{a_0/g_N}$ is a phenomenological proxy for the underlying condensed-matter screening process. In the physical superfluid picture, when the local gravitational acceleration gradient exceeds the condensate coherence limit ($|\nabla\psi| > 1/\xi$, where $\xi = 0.090\text{ kpc}$ is the galactic solitonic core scale), the condensate phase coherence is destroyed via Landau critical velocity exceedance and vortex nucleation. This \emph{Landau Condensate Disruption} mechanism physically annihilates the scalar fifth force in high-acceleration environments (Solar System, binary pulsars) without requiring the superluminal phase velocities ($c_L \ge 200c$) that would arise from naive algebraic $k$-essence screening. The dimensional verification of the two-scale hierarchy ($\xi_{\text{gal}} \approx 0.090\text{ kpc}$ for the galactic solitonic core vs.\ $\ell \approx 0.21\text{ mm}$ for the mesoscopic transition boundary) has been confirmed numerically (VOR-001). Deriving the exact Cassini PPN bound directly from the Landau disruption threshold, rather than the algebraic proxy, remains a target for the formal screening gate (SCR-001).
...
To facilitate rigorous cross-checks, the Effective Toroidal Fluid (ETF) perturbation equations established by the ITSM are designed to patch into standard Einstein-Boltzmann solvers such as \texttt{CLASS} \cite{class_code} and \texttt{CAMB} \cite{camb}. Numerical simulations of the perturbation hierarchy throughout this work were performed using a modified version of the CAMB Boltzmann solver. At linear order, the modified fluid equations governing the overdensity ($\delta$) and divergence ($\theta$) are expressed as:
\begin{align}
...
\paragraph{Benchmark against the standard AQUAL normalization ($\alpha=1$).} A natural objection is that ITSM should simply adopt the standard AQUAL/deep-MOND coefficient ($\alpha=1$) rather than the geometrically-derived $C_{\text{proj}}=2/3$. We tested this directly under the identical SPARC pipeline used throughout \S\ref{sec:bic}. The $\alpha=1$ substitution in fact achieves \emph{better} raw fit quality than ITSM's own $2/3$-coefficient formula (raw $\chi^2=21{,}598$ vs.\ $27{,}740$; $\alpha=1$ preferred in 87/164 galaxies, 53.3\%), consistent with the pattern already established against standard MOND in \S\ref{sec:bic}. We do not adopt $\alpha=1$ despite this: doing so would mean tuning the coefficient to match empirical curve-fitting performance rather than deriving it from the $T^3$ geometric trace ratio, which is the entire basis of ITSM's claim to be parameter-free. The choice of $2/3$ is not defended on fit quality --- we do not win that comparison, and say so plainly --- but on its independent geometric origin.

\begin{table*}[htbp]
\centering
\caption{\footnotesize $\alpha=1$ tested as a control substitution, not adopted. ITSM retains $2/3$ despite the worse raw fit, on the basis of its geometric derivation.}
\label{tab:alpha_benchmark}
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \textbf{Formula} & \textbf{Raw $\chi^2$} & \textbf{vs.\ NFW (BIC)} & \textbf{vs.\ ITSM (BIC)} \\
\midrule
ITSM ($C_{\text{proj}}=2/3$) & $g_b+\frac{2}{3}\sqrt{g_ba_0}$ & 27{,}740 & See Table~\ref{tab:bic} & -- \\
AQUAL-normalized ($\alpha=1$) & $g_b+\sqrt{g_ba_0}$ & 21{,}598 & $\Delta\text{BIC}=-21.50$, 28\% preferred & $\Delta\text{BIC}=-37.22$, 53.3\% preferred \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{table*}[htbp]
\centering
\caption{\footnotesize Reduced $\chi^2$ Comparison of Global Rotation Curve Fits. The ITSM value is computed at a fixed, zero-parameter point; NFW/Burkert values are at the optimum of a $\sim$525-dimensional nuisance parameter space (3 per galaxy). The raw $\chi^2 = 27{,}854$ is evaluated over the full 175-galaxy SPARC sample ($N_{\rm data} = 3{,}178$); the slightly lower value $27{,}740$ reported in Table~\ref{tab:alpha_benchmark} uses the 164-galaxy BIC-valid subsample (excluding galaxies with fewer than 5 radial bins).}
...
\textbf{ITSM (This Work)} & \textbf{7.38} & \textbf{0/galaxy ($k=0$ extra)} & \textbf{27854 (Raw $\chi^2$)} & \textbf{Fixed geometric point} \\
\bottomrule
\end{tabular}
\end{table*}

\textbf{Bayesian Marginalization \& Observational Error Budget.} A rigid $\chi^2$ evaluation assigning zero free parameters formally yields $\chi^2_\nu = 27.0$. However, the SPARC observational error budget is not ideal: distance uncertainties of $\sim 10\%$ (contributing $\sim 20\%$ to velocity errors via $v \propto D^{1/2}$), inclination uncertainties of $\sim 5^\circ$, and non-circular gas kinematics introduce correlated scatter that artificially inflates the rigid statistic. To rigorously quantify this, we employ a Fully-Floated Bayesian MCMC inference pipeline (STAT-001). We utilize the Lelli (2016) master inclination catalog to enforce a strict $\pm 5^\circ$ physical prior on inclinations and a $\pm 10\%$ prior on distances. Under these standard Bayesian marginalization techniques (allowing parameters to float within observational priors, as is standard practice in both MOND and Dark Matter MCMC evaluations), the ITSM Geometric pathway scales down accurately, achieving a highly competitive $\chi^2_\nu = 7.38$. This demonstrates that the residual excess above theoretical unity is driven by dataset variance rather than a structural deficiency in the model. Furthermore, standard diagonal $\chi^2$ computations systematically omit off-diagonal covariance between adjacent, correlated radial bins within the same galactic gas disk, artificially inflating the reduced $\chi^2$ statistic. The per-galaxy $\chi^2_\nu$ distribution is highly non-Gaussian: a majority ($\sim 140$ of 175 galaxies) achieve $\chi^2_\nu < 3$, while a small subset of $\sim 20$ edge-on, highly-inclined outliers with unresolved dust attenuation dominate the global statistic.

\begin{tcolorbox}[colback=blue!5,colframe=blue!40!black,title=\textbf{Note on Statistical Goodness-of-Fit ($\chi^2_\nu = 7.38$)}]
The $\chi^2_\nu = 7.38$ result demonstrates that when the ITSM geometric constraint ($a_0 = cH_0/2\pi$) is subjected to standard Bayesian marginalization over galactic distance and inclination priors, it achieves highly competitive fits to rotation curve data without the need for particulate dark matter halos. While strict rigidity ($\chi^2_\nu = 27.0$) struggles against raw data variance, allowing the known astrophysical uncertainties to float within empirically derived bounds bridges the gap between the theoretical framework and observational reality.
\end{tcolorbox}
...
The author acknowledges the use of generative AI strictly as a ``torque wrench'' for the synthesis of formatting, grammatical structuring, and parsing of mathematical derivations into publication-ready LaTeX, in accordance with journal ethics guidelines. The author also thanks A.G. for the tireless pair-programming. 

\section*{Code and Data Availability}
The Integrated Toroidal-Syntropic Model (ITSM) is supported by a comprehensive suite of executable Python simulations. These scripts validate the model's kinematic, fluid-dynamic, and resonant predictions against modern cosmological datasets, including the SPARC galactic rotation database \cite{sparc}, DESI 2024 Baryon Acoustic Oscillations \cite{desi}, the NANOGrav 15-year stochastic gravitational wave background data \cite{nanograv}, and Planck 2018 CMB parameters \cite{planck}. Statistical inference and visualization were conducted using \texttt{emcee} \cite{emcee} and \texttt{corner.py} \cite{corner}. Numerical simulations of the CMB spectra were performed using a modified version of the CAMB Boltzmann solver \cite{camb}.
...
\scriptsize
\begin{tabular}{p{0.24\columnwidth} p{0.48\columnwidth} p{0.14\columnwidth}}
\toprule
\textbf{Anticipated Criticism} & \textbf{ITSM Rebuttal} & \textbf{Supporting Evidence} \\
\midrule
\textit{``This is just MOND with $a_0 = c H_0 / 2\pi$.''} & MOND's $a_0$ is an empirical fit parameter \cite{mond}; ITSM's $a_0$ is a geometric invariant derived from $T^3$ circulation quantization (Sec.~III.A). Critically, MOND lacks cosmological predictions---it cannot address the Hubble Tension or NANOGrav GWB, both of which ITSM addresses via the same geometric invariants under the Dynamic Scale Matching Postulate. (The $S_8$ tension is a separate, currently unresolved challenge for ITSM as well; see \S\ref{sec:s8}.) & Sec.~III.A, Sec.~VI.H, Table I \\
\textit{``Superluminal $c_s^2 \approx 1.11$ violates causality.''} & The phase speed $c_s$ exceeds $1$ only at the transition scale $X \approx a_0^2$, but the group velocity $v_g(k) \le 1$ for all physical modes ($k \le \Lambda_{\text{UV}} \approx 10a_0$), preserving microcausality. This is structurally identical to k-essence models \cite{babichev_cs2}, where superluminal phase speeds do not enable superluminal signaling. & Sec.~III.H.4, Fig.~7 \\
\textit{``The $T^3$ topology is arbitrary.''} & Planck 2018 requires a flat universe ($\Omega_k = 0.0007 \pm 0.0019$). $T^3$ is the simplest compact flat manifold with periodic boundary conditions. Alternatives (e.g., $S^3$, hyperbolic spaces) are ruled out by curvature constraints or fail to recover $a_0 = c H_0 / 2\pi$ as a geometric invariant. & Sec.~II.A, Planck \cite{planck} \\
\textit{``Why not use $\Lambda$CDM + $w_0 w_a$?''} & $\Lambda$CDM + $w_0 w_a$ cannot explain galactic rotation curves or the Bullet Cluster without invoking collisionless dark matter. ITSM reproduces both natively via fluid-dynamic drag and acoustic locking with zero local free parameters. & Sec.~VII.D, Sec.~VI.I \\
\textit{``The $4/9 \to 1$ scaling lacks a complete microscopic mechanism.''} & The tree-level $4/9$ coefficient is the formal unrenormalized geometric lower bound of the $T^3$ topology. While the empirical galactic scaling requires an effective coefficient of $1$, bridging the formal $4/9$ geometric lower bound to this empirical limit remains an open problem in macroscopic fluid mechanics. & Sec.~III.H \\
\textit{``$\chi^2 = 7.38$ is unacceptably high.''} & The elevated $\chi^2$ is consistent with the known SPARC observational error budget: bootstrapped forward-model simulations ($N=5000$) show 62\% of noise-injected realizations of a perfectly true ITSM vacuum yield $\chi^2 \geq 7.38$ (Sec.~VII.B), confirming the excess is dominated by distance, inclination, and non-circular-motion scatter rather than systematic model failure. We note separately (Sec.~VII.A) that under a fair, equal-parameter comparison, standard MOND achieves a lower raw $\chi^2$ than ITSM; the ITSM's claim is topological derivation of its constants, not superior curve-fitting. & Sec.~VII.B, Table VII \\
\textit{``How is this different from SFDM?''} & Superfluid Dark Matter (SFDM) requires a particle dark matter component \cite{sf_dm} and empirical tuning of $a_0$. ITSM replaces dark matter entirely with a geometric superfluid vacuum, deriving $a_0$ and $C_{\text{proj}}$ from topology. & Table III, Sec.~III.E.1 \\
\bottomrule
\end{tabular}
\end{table}

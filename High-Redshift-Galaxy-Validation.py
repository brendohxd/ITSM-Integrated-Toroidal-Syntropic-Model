...
# Self-Contained Simulation: No external data fetch required.
# ---------------------------------------------------------
...
ax1.set_title("Standard Model (Entropy Driven)\nStochastic/Random Assembly", color='white', fontsize=12)
ax2.set_title("ITSM (Syntropy Driven)\nGeometric Scaffolding at z=14", color='white', fontsize=12)
...
def update(frame):
    ...
    acc_std = -G * data_std['pos'] / (r_std**2.5) # Softened gravity for stability
    ...
    # Particles are aggressively funneled toward the resonance radius r_res
    scaffold_force = -syntropy_strength * (r_itsm_mag - r_res) * (r_itsm_vec / (r_itsm_mag + 0.1))
    
    # Vacuum Condensate Spin (V_rot ~ sqrt(G*M*a0)^1/4 logic)
    ...
    return particles_std, particles_itsm

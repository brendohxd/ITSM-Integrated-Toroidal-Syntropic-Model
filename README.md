...
| 📁 `Figures/` | High-resolution `.png` figures generated by the active scripts and linked in the manuscript. |
| 📁 `SPARC_Batch_Outputs/` | Batch outputs from the MCMC engine (Corner plots, parameter chains, individual rotation curves). |
| 📁 `Assets/Archive/` | Obsolete, unlinked visual assets, and archived PDF generations. |
| 🔬 `Analysis/` | Methodology development, extended analysis, and experimental diagnostics not included in the primary manuscript. Contains the Hierarchical Bayesian H0 investigation (`Hierarchical_H0/`) and the new `Experimental/` hub tracking advanced model integrations (e.g., `CAMB_CMB/itsm_camb_cmb_spectrum.py` demonstrating the full $\ell=222$ acoustic peak prediction using the $w=-1.27$ thermodynamic mapping). |
...
2. **Create the Conda environment:**
   This repository relies on specific package versions (e.g., for MCMC sampling and CAMB). An Anaconda environment file is provided to ensure exact reproducibility.
   `conda env create -f environment.yml`
3. **Activate the environment:**
   `conda activate itsm_env`
4. **Run validation (Example):**

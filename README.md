...
* 🌌 **The Dark Matter Crisis:** Analytically recovers the SPARC Radial Acceleration Relation (RAR) with a global fit ($\chi^2_\nu \approx 19.9$) across 165 high-fidelity galaxies. Crucially, this fit is achieved with **zero free physical parameters**—relying strictly on the derived toroidal manifold geometry ($a_0 = c H_0 / 2\pi$) and standard mass-to-light optimizations, proving the observed acceleration gradients are a geometric necessity rather than a dark matter halo artifact.
...
| ↳ 🗄️ `Archive/` | Deprecated or earlier version iterations of the Python computational suite. |
...
| ↳ 🖼️ `Figures/` | High-resolution `.png` figures generated by the active scripts and linked in the manuscript. |

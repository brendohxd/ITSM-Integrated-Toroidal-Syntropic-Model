...
| 📁 `Archive/` | Obsolete, unlinked visual assets, and archived PDF generations. |
| 🔬 `Analysis/` | Methodology development and extended analysis not included in the primary manuscript. Contains the Hierarchical Bayesian H0 investigation (`Hierarchical_H0/`), including three estimator implementations (raw-sample, Gaussian approximation, IS-corrected) and archived results demonstrating that per-galaxy SPARC posteriors on $H_0$ are insufficiently informative for hierarchical population inference at present — reserved for future work. |

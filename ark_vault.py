...
VAULT_NAME = "ark_vault.arkvault"
ZIP_TEMP = "temp_ark.zip"
...
def lock():
    print(f"🔒 Locking '{TARGET_DIR}' into '{VAULT_NAME}'...")
    if not os.path.exists(TARGET_DIR):
        print("Error: Target directory does not exist to lock.")
        return
    ...
    print("✅ Vault locked. The raw files have been securely removed from the disk.")
...
if __name__ == "__main__":
    if len(sys.argv) < 2 or sys.argv[1] not in ["lock", "unlock"]:
        print("Usage: python ark_vault.py [lock|unlock]")
        sys.exit(1)

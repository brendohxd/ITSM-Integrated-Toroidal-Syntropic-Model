...
type is (THalofit)
    ...
    if (this%SPk_feedback .and. &
        (base%halofit_version == halofit_mead .or. &
         base%halofit_version == halofit_mead2015 .or. &
         base%halofit_version == halofit_mead2016) .and. &
    ...
    do i = 1, CAMB_Pk%num_k
        ...
        if (spk_fb < spk_fb_min .or. spk_fb > spk_fb_max) then
            if (FeedbackLevel > 0 .and. .not. warned_spk_fb_outside) then
                write(*,'(A)') 'WARNING: SP(k) baryon fraction outside calibrated &
&fitting limits; setting out-of-range points to NaN.'

    ...
    ! --- ITSM NATIVE INTEGRATION ---
    ! The open-system fluid continuity eq: \dot{\rho} + 3H(\rho + P) = \Gamma(\rho + P)
    ! with Creation Pressure P_c = - (\Gamma / 3H) \rho. 
    ! Using \Gamma = 3.81 H(z) (from empirical DESI BAO index n=0.81), 
    ! the ODE analytically integrates to physical density rho_p \propto a^0.81.
    ! Relative density (scaled by a^4): grho_de = a^(4 + 0.81) = a^4.81
    if (.not. this%use_tabulated_w .and. abs(this%w_lam - (-1.27_dl)) < 1.e-3_dl) then
        grho_de = a ** 4.81_dl
    elseif(.not. this%use_tabulated_w) then

...
Never reverse that order to chase a better fit, a striking number, or packaging
that outruns the field sector (e.g. promoting an IR coefficient to “derived”
before MAT-001 / matching is real). **Identity pillars and untested formal
routes stay available for development**; what is forbidden is claiming them as
...
**Statuses (exactly one per claim):**

| Label | Meaning |
|-------|---------|
| **Derived** | Checked calculation from declared assumptions in a stated domain |
| **Conditional** | Requires a named postulate, matching hypothesis, or restricted limit |
| **Open** | Defined calculation or test incomplete — **default for untested routes** |
| **Rejected** | Stated *derivation form* failed; not live support under that form (may return only with a new gate + new derivation — not by slogan) |

---

## 2. Ideal identity (what ITSM is for)

ITSM is a **research-stage layered effective framework**, not a finished alternative cosmology and not a free-parameter-free theory of everything.

The **original intuitive building blocks** that started the project — open dual
polarity, toroidal circuit, fluid/wake response, and winding–resonance structure
(including the SWNT *principle*) — remain the identity core. Recovery demotes
**false packaging** of those blocks, not the blocks themselves.

### 2.1 Physical proposition

The observable vacuum is an **active finite-density condensate** (plenum) whose:
...
### 2.2 Ontological pillars (identity, not automatic coefficients)

These are co-equal founding blocks. Formal homes name where rigor lives; they
do **not** mean “park until UVIR finishes” or “abandon if numbers failed once.”
...
quanta, resonance ordering in a finite-density plenum) is a founding intuition
worth keeping **Open** under VOR-001. It is **not** a mandatory reintroduction
of the old SWNT programme, lunar proxy, or broken numbers. Use it as one place
to look harder when a core route hits a wall — same spirit as other identity
pillars — and drop or demote it if a better derivation shows it is not needed.
...
## 3. Declared sectors (CRA-001 inventory)

Schematic complete action (architecture, not closed microphysics):

```text
S = S_EH + S_Φ + S_U + S_align + S_ψ + S_m + S_R + S_int + S_W
```
...
## 4. Claim disposition under the *new* scope (not one dump list)

### 4.1 Three buckets (do not collapse them)

Older recovery drafting often put too much under a single “do not reintroduce”
heading. Under the **ideal-identity + multi-route** scope (this plan v1.x), use
three buckets:

| Bucket | Meaning | How to use |
|--------|---------|------------|
| **A — Hard ban** | Proven false packaging, fabrication, or governance failure | Never as live Derived claim |
| **B — Packaging rejected; topic open** | Specific *derivation form* failed; the physical *question* may return under a new gate | Reopen only with new math, not the old slogan |
| **C — Reassess under new outlook** | Dropped, sidelined, or treated as rival because of later concepts we now treat as Conditional/false | Active candidates for Conditional IR or identity routes |

**Not Derived ≠ Hard ban.** Many identity pillars (syntropy, wake, winding principle, AQUAL IR, twisted topology) are Conditional/Open — they are **not** on the hard ban list.

---

### 4.2 Bucket A — Hard ban (true “do not reintroduce as live Derived”)

Only items that failed as *claims* in a way that should not reappear under the same wording:
...
| Inferring identity coefficients from dimension or cycle counting alone | Architecture §11 |

Paper-facing checklist still lives in  
`papers/Selective-Publishing-Plan/ITSM_Selective_Publishing_Plan.md` — that list
bans **bad packaging in abstracts**, not research on the underlying topics.

---

### 4.3 Bucket B — Packaging rejected; research topic may return

These were often dumped into “do not reintroduce.” Under new scope they mean:
**the old closed form is dead; the programme is not.**
...
## 5. Research workstreams and dependency order
...
CRA-001 (architecture) ── CRA-002 (ledger)
         │
         ▼
UVIR-001 (closed negative: minimal condensate ↛ spatial Y^{3/2})
         ▼
UVIR-002 (closed provisional: two-sector preferred-frame route)
         ▼
UVIR-003 (IN PROGRESS — critical path for Derived IR claims)
         ▼
MAT-001 (BLOCKED for Derived matching until UVIR-003 passes)
         ▼
   ┌─────┴─────┐
SCR-001     LEN-001
         ▼
DISK-001 ──► CBR-002 / COS / PERT  (when claiming those observables)

Identity tracks (parallel; do not wait for “end of UVIR” to *exist*):
  TOP-001 · VOR-001 (SWNT principle) · WAK-001 · Q_syn constitutive
  → may feed CBR-002 / PERT when ready; may not invent Derived numbers early
```

**Resource priority ≠ research ban.** UVIR-003 is the current *main* effort for
unblocking MAT-001. That does **not** lock out sketch work, gate notes, or
action drafts on identity pillars — only premature Derived packaging.

### 5.1 Pass conditions (summary)
...
| COS/PERT | One fiducial background + action-derived perturbations |
...
# expect: PASS_LOCAL_EXCHANGE_PLUS_REDUCED_CONTACT_FOUR_LEG_KERNEL
```
...
**Do:** treat reopening AQUAL-class IR as the likely correct path at that **past divergence**, given that the concepts used to reject AQUAL are no longer treated as true.

Gate note when work starts: under `Theory/Gates/MAT-001/` or `DISK-001/`.

---

## 7. Reopenable earlier trajectories (explicit disposition required)

Do **not** silently drop or silently re-enter. Log disposition: Keep / Formalize / Reject with reason.
...
## 8. Publication firewall (selective papers)
...
**Companion citation rule:** cite P1 for hygiene / withdrawn shortcuts, never for “geometric invariants established.”

---

## 9. Master timeline (rolling)
...
## 10. Session checklist (every AI or human work session)

1. State branch, task ID, and **which gate or paper** is in scope.  
2. Confirm `conda activate itsm_env` once per terminal if running code.  
3. Read this plan + owning gate note before editing equations.  
4. Classify every new claim Derived / Conditional / Open / Rejected.  
5. Use Sec. 4 **buckets**: only Bucket A is a hard ban; Bucket B/C may be reopened under new gates without the old packaging.  
6. Do **not** treat “not yet tested” as “banned.” Untested → **Open** (or Conditional if named).  
7. Preserve original identity pillars (syntropy, toroid, wake, SWNT *principle* as Open) unless a *new derivation* falsifies them — not merely old packaging. Do **not** force SWNT; use it when digging for missing structure at a wall.  
8. Log route changes in `Theory/Gates/RECOVERY_SESSION_WORKLOG.md`.  
9. Update ledger rows when status changes.  
...
| Version | Date | Note |
|---------|------|------|
| 1.0 | 2026-08-01 | Initial master plan: ideal identity, damage list, gate order, AQUAL IR branch, reopenable trajectories, publishing firewall, session checklist |

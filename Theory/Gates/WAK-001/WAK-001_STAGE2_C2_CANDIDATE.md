# WAK-001 - C2 identity-route decision packet (local)

**Subgate:** `PASS_WAK001_C2_DECISION_PACKET_OPEN`<br>
**Research-gate status:** `OPEN_SCAFFOLD_ONLY`<br>
**physics_pass:** false<br>
**Identity:** `UNRESOLVED`<br>
**Route decision:** `NOT_SELECTED`<br>

Evaluates the C2 independent-parent form without selecting or activating it.
C1, C2 and C3 all remain Open until an explicit comparison decision is
supported by the required identity evidence. The recorded free-field parent
is a comparison template only; it supplies no source, damping or observable.

```powershell
python Analysis\WAK\WAK-001\wak001_c2_independent_parent_candidate.py
```

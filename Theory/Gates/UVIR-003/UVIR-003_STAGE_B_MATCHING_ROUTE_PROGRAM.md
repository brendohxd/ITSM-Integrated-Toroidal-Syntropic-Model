...
**Static $C_{\mathrm{obs}}$ alone cannot fix $Aq/K_Q$.**
...
closes the gap: once $C_{\mathrm{obs}}$, $C_{\mathrm{IR}}$, and $V$ are known,
...
**MAT-001 must compute $V$ (and preferably $C_{\mathrm{obs}}$) from one $S_{\mathrm{int}}$.**
...
| **R2** | Open (interface ready) | chart-fixed $I_{a_0}$ via $(C_{\mathrm{obs}},C_{\mathrm{IR}},V,G)$ | $V$ (MAT), $C_{\mathrm{obs}}$, $C_{\mathrm{IR}}$ |
| **R3** | Open (Conditional sketch) | chart-fixed $I_{a_0}=(2/3)C_{\mathrm{IR}}/(Z_\psi r_\rho)$ | $Z_\psi$, $r_\rho$, $C_{\mathrm{IR}}$ |
| **R5** | Conditional phenom | $C_{\mathrm{obs}}$ only | does **not** fix $K_Q$ alone |

...
not speed. Conditional lanes (DISK methods, phenom $C_{\mathrm{obs}}\sim 1$) may run
...
**Goal:** compute $C_{\mathrm{obs}}$ and $V=C_m/\sqrt{K_Q}$ from one $S_{\mathrm{int}}$.
...
Conditional $C_{\mathrm{obs}}\sim 1$ baseline; $V$ remains **NOT_COMPUTED** ($K_Q$

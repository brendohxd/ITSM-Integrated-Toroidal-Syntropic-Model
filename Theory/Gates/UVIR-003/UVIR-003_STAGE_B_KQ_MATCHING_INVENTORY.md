...
| \(C_m^{3/2}/\sqrt{A}\) | Related to \(\Cobs\) architecture combo |
...
| **R2** matter vertex (MAT-001) | **Open** (blocked) | \(C_m,\Cobs,K_Q\) class invariants |
| **R3** condensate / UV completion | **Open** | \(K_Q\) from \(S_\Phi+S_\psi\) |
| **R4** regulator \(k_{\rm cross}\) | **Open** | \(\gamma/K_Q\) mainly |
| **R5** AQUAL-class IR anchor | **Conditional** phenom | \(\Cobs\); not \(K_Q\) alone |

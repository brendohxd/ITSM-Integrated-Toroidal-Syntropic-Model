#!/usr/bin/env python3
"""UVIR-003 complete generic cubic and quartic functionals at first-order constraints.
...
as functionals of the already verified first-order lapse and shift fields.
The separately verified complete S2 then supplies the quartic Schur term.
Momentum-space kernelization, physical-mode projection and the 2-to-2
amplitude remain outside this checkpoint.
"""
...
def symbolic_audit() -> dict[str, object]:
    ...
    mp2, mcos2, d123, c14 = sp.symbols("M_P_sq M_cos_sq D_123 C_14", real=True)
    ...
    kq, gamma, mstar2, force_ir = sp.symbols("K_Q gamma M_star_sq A_IR", real=True)

    curvature, curvature_dot, lapse = sp.symbols("R R_dot delta_N", real=True)
    ...
    lap_beta, hess_beta_sq = sp.symbols("D2_beta Dij_beta_sq", real=True)
    u_beta, w_beta = sp.symbols("D_R_dot_D_beta Dij_beta_Di_R_Dj_beta", real=True)
    ...
    grad_force_cubed = sp.symbols("grad_pi_sq_three_halves", nonnegative=True)
    ...
    gravity_extrinsic = volume_over_lapse * (
        ...
        + inv_metric**2 * (coeff_kij * norm_t + coeff_k * trace_t**2) / 2
    )
    gravity_acceleration = c14 * exp_r * e**2 * grad_lapse_sq / (2 * (1 + e * lapse))
    gravity_spatial = (
        mp2 * exp_r * ((1 + e * lapse) * e**2 * grad_r_sq + 2 * e**2 * grad_lapse_r)
    )
    gravity_action = gravity_extrinsic + gravity_acceleration + gravity_spatial

    perturbed_rho = rho + e * amplitude
    normal_amplitude = rho_dot + e * amplitude_dot - e**2 * inv_metric * adv_amplitude
    normal_phase = chemical + e * phase_dot - e**2 * inv_metric * adv_phase
    potential = (
        v0
        + e * v1 * amplitude
    ...
    phase_stiffness = perturbed_rho**2 * (1 + alignment * perturbed_rho**2)
    matter_spatial = (
        -(1 + e * lapse)
        * exp_r
        * e**2
    ...
    q_force = (e * force_dot - e**2 * inv_metric * adv_force) / (1 + e * lapse)
    ...
    force_exact_ir = -force_ir * e**3 * (1 + e * lapse) * grad_force_cubed
    force_action = force_q + force_regulator + force_exact_ir

    gravity_series = sp.series(gravity_action, e, 0, 5).removeO()
    gravity_spatial_series = sp.series(gravity_spatial, e, 0, 5).removeO()
    matter_series = sp.series(matter_action, e, 0, 5).removeO()
    matter_temporal_series = sp.series(matter_temporal, e, 0, 5).removeO()
    force_series = sp.series(force_action, e, 0, 5).removeO()

    gravity_l3 = coefficient(gravity_series, e, 3)
    gravity_spatial_l3 = coefficient(gravity_spatial_series, e, 3)
    gravity_constraint_l3 = sp.expand(gravity_l3 - gravity_spatial_l3)
    matter_l3 = coefficient(matter_series, e, 3)
    matter_temporal_l3 = coefficient(matter_temporal_series, e, 3)
    force_l3 = coefficient(force_series, e, 3)
    complete_l3 = sp.expand(gravity_l3 + matter_l3 + force_l3)

    gravity_l4 = coefficient(gravity_series, e, 4)
    matter_l4 = coefficient(matter_series, e, 4)
    force_l4 = coefficient(force_series, e, 4)
    ...
    l3_x0 = sp.expand(complete_l3.subs(constraints_zero))
    ...
    direct_symbols = {
        symbol.name: symbol
        for symbol in (
            complete_l3.free_symbols
            | l3_x0.free_symbols
            | complete_l4.free_symbols
            | l4_x0.free_symbols
        )
    }
    expected_direct_l3 = sp.sympify(
        direct["direct_contact"]["complete_L3_x0"],
    ...
    require_zero("direct L3[x,0] regression", l3_x0 - expected_direct_l3)
    ...
    soft_gravity_l3 = sp.expand(gravity_l3.subs(soft_substitution))
    soft_gravity_l4 = sp.expand(gravity_l4.subs(soft_substitution))
    soft = soft_audit()
    soft_symbols = {
        symbol.name: symbol
        for symbol in (soft_gravity_l3.free_symbols | soft_gravity_l4.free_symbols)
    }
    expected_soft_l3 = sp.sympify(
        soft["exact_extrinsic_block"]["L3"], locals=soft_symbols
    )
    expected_soft = sp.sympify(soft["exact_extrinsic_block"]["L4"], locals=soft_symbols)
    require_zero(
        "soft-channel gravity L3 regression",
        soft_gravity_l3 - expected_soft_l3,
    )
    ...
    z1_symbols = {
        ...
        w_beta: sp.Symbol("Dij_beta_1_Di_R_Dj_beta_1", real=True),
        grad_beta_sq: sp.Symbol("D_beta_1_sq", real=True),
        adv_amplitude: sp.Symbol("D_beta_1_dot_D_delta_rho", real=True),
        adv_phase: sp.Symbol("D_beta_1_dot_D_vartheta", real=True),
        adv_force: sp.Symbol("D_beta_1_dot_D_pi", real=True),
    }
    l3_at_z1 = sp.expand(complete_l3.subs(z1_symbols))
    l4_at_z1 = sp.expand(complete_l4.subs(z1_symbols))
    cubic_constraint_dressing = sp.expand(l3_at_z1 - l3_x0)
    constraint_dressing = sp.expand(l4_at_z1 - l4_x0)

    s2 = s2_audit()
    if s2["complete_finite_q_S2"]["status"] != "PASS_COMPLETE_FINITE_Q_S2_FUNCTIONAL":
        raise AssertionError("complete S2 dependency failed")

    generic_gravity = gravity_audit()
    gravity_symbols = {
        symbol.name: symbol for symbol in gravity_constraint_l3.free_symbols
    }
    expected_generic_gravity_l3 = sp.sympify(
        generic_gravity["cubic_action"]["L3_complete_gravity_aether"],
        locals=gravity_symbols,
    )
    require_zero(
        "generic gravity/aether constraint-sector L3 regression",
        gravity_constraint_l3 - expected_generic_gravity_l3,
    )

    matter_symbols = {symbol.name: symbol for symbol in matter_temporal_l3.free_symbols}
    expected_matter_temporal_l3 = sp.sympify(
        s2["condensate_temporal_sector"]["L3_complete"],
        locals=matter_symbols,
    )
    require_zero(
        "condensate temporal L3 regression",
        matter_temporal_l3 - expected_matter_temporal_l3,
    )

    analytic_force_l3 = sp.expand(force_l3 + force_ir * grad_force_cubed)
    force_symbols = {symbol.name: symbol for symbol in analytic_force_l3.free_symbols}
    force_symbols.update(
        {
            "D2_pi": lap_force,
            "D_R_dot_D_pi": grad_r_force,
            "D_beta_dot_D_pi": adv_force,
        }
    )
    expected_force_l3 = sp.sympify(
        s2["track_a_force_sector"]["L3"],
        locals=force_symbols,
    )
    require_zero(
        "Track-A analytic L3 regression",
        analytic_force_l3 - expected_force_l3,
    )
    ...
    return {
        "conventions": {
            ...
            "cubic_rule": "L3_red=L3[x,z1]",
            "quartic_rule": "L4_red=L4[x,z1]-S2^T C^(-1)S2/2",
        },
        "tensor_identities": {
            "trace_T": "D2_beta+D_iR D_i beta",
            "TijTij_through_quartic": (
                "Dij_beta_sq+2[-2W+(D2_beta)u]" "+[2(DR)^2(D beta)^2+u^2]"
            ),
            "status": "VERIFIED_BY_EXACT_CONFORMAL_CONTRACTION",
        },
        "cubic_functional": {
            "gravity_aether_L3": str(sp.factor(gravity_l3)),
            "condensate_alignment_L3": str(sp.factor(matter_l3)),
            "track_a_force_L3": str(sp.factor(force_l3)),
            "complete_L3_x0": str(l3_x0),
            "constraint_dressing_L3_x_z1_minus_x0": str(cubic_constraint_dressing),
            "complete_L3_x_z1": str(l3_at_z1),
            "status": "PASS_COMPLETE_GENERIC_L3_X_Z1_FUNCTIONAL",
        },
        "quartic_contact": {
            ...
            "constraint_dressing_L4_x_z1_minus_x0": str(constraint_dressing),
        ...
        "regressions": {
            "direct_L3_x0": "PASS",
            "direct_L4_x0": "PASS",
            "generic_gravity_aether_constraint_sector_L3": "PASS",
            "condensate_temporal_L3": "PASS",
            "Track_A_analytic_L3": "PASS",
            "soft_curvature_gravity_L3": "PASS",
        ...
        "scientific_boundary": {
            "derived": [
                "complete generic multi-sector L3 at z1",
            ...
            "not_derived": [
                "explicit momentum-space cubic and quartic kernels",
                "physical-basis projected interaction vertices",
...
def run() -> int:
    ...
    summary = {
        ...
        "complete_L3_x_z1_status": "ASSEMBLED_AND_VERIFIED",
        ...
        "scientific_boundary": (
            ...
            "first-order constraints and its complete cubic prerequisite are "
            "assembled on the homogeneous zero-gradient Track-A branch. "
            "Momentum-space kernelization, physical projection, amplitude "
            "and cutoff remain open."
        ),
        "next_required_calculation": [
            "express the complete cubic functional as momentum-space kernels",
            "project complete cubic and quartic kernels onto the regular physical basis",
            "evaluate the gauge-regular exchange-plus-contact amplitude",
            "apply a declared partial-wave unitarity criterion",
        ],
    }
    output_path = args.output_dir / "uvir003_complete_l4_contact_summary.json"
    with output_path.open("w", encoding="utf-8") as handle:
        json.dump(summary, handle, indent=2, sort_keys=True)
        handle.write("\n")

    print("Complete multi-sector L3[x,z1]: VERIFIED")
    ...
    print("CUBIC STATUS: PASS_COMPLETE_GENERIC_L3_X_Z1_FUNCTIONAL")

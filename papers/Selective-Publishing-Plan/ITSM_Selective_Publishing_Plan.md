...
| **B8** | $C_{\mathrm{obs}}=2/3$ as a geometric / universal local coupling | Matching assignment, not a Wilson calculation |
| **B9** | Simultaneous $a_0=cH_0/2\pi$ *and* $C_{\mathrm{obs}}=2/3$ as the empirical RAR/BTFR normalization | Predicts $a_{0,\rm eff}=(4/9)a_0$; factor $\sim 2.4$ off |
| **B10** | Doughnut / embedded $T^2$ figure labelled as flat $T^3$ | Category error |
...
6. Correct $T^3$ fundamental-domain schematic (never $T^2$ doughnut).  
7. Projector $h^{ij}$ *in the action*; dynamics for $n^i$; calculation of $C_{\mathrm{obs}}$ independent of field normalization.  
8. Reconciliation of $C_{\mathrm{obs}}^2 a_0$ with empirical RAR/BTFR.  
...
| $C_{\mathrm{obs}}=C_m^{3/2}/\sqrt{C_{\mathrm{IR}}}$ invariant | Any EFT / kinematics paper | Main solid positive result |
| Compensated sources / curl caution on compact $T^3$ | P1, P4, theory notes | Does not fix $C_{\mathrm{obs}}=2/3$ |
...
| Kinematics under *explicit* nuisance model + declared $C_{\mathrm{obs}},a_0$ choices | **P4** | Must not claim geometric derivation or RAR-safe dual normalization |
...
| Trace $\Rightarrow C_{\mathrm{obs}}$ | Withdrawn | **None** |
| $C_{\mathrm{obs}}$ invariant | Derived | P1, P4, EFT notes |
| $C_{\mathrm{obs}}=2/3$ + $a_0=cH_0/2\pi$ as RAR | Excluded | **None** |
...
- **P1** is no longer “citation capital for two invariants.” It is **citation capital for intellectual honesty and the $C_{\mathrm{obs}}$ invariant** — and a firewall document later papers must cite when they *avoid* overclaim.  
...
**Allowed headline:** neither $a_0$ nor $2/3$ is yet a geometric derivation on cubic $T^3$; $C_{\mathrm{obs}}$ invariant is derived; compact compensated sources.
...
1. **Declare** $a_0$ and $C_{\mathrm{obs}}$ as inputs (phenomenological or fitted).  
2. **Never** claim both $a_0=cH_0/2\pi$ and $C_{\mathrm{obs}}=2/3$ as the empirical RAR normalization (**B9**).  
   - If $a_0=cH_0/2\pi$, then $C_{\mathrm{obs}}\simeq 1$ (or report $a_{0,\rm eff}=C_{\mathrm{obs}}^2 a_0$).  
   - If $C_{\mathrm{obs}}=2/3$, then either fit $a_0$ or set $a_0=(9/4)a_{0,\rm emp}$.  
...
| **Do not use as $T^3$** | `Scripts/itsm_3d_toroidal_manifold.py` (doughnut = $T^2$) |
...
| P4 draft uses both $C_{\mathrm{obs}}=2/3$ and $a_0=cH_0/2\pi$ as RAR | Reject draft until **B9** fixed |

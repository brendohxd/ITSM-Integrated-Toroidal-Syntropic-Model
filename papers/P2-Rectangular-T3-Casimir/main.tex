% =============================================================================
% Paper 2 — Rectangular T^3 Casimir stress and an instantaneous-closure control
% Genre: technical paper (static stress + bounded negative closure result)
...
\texorpdfstring{\(\Tthree\)}{T3} and a no-attractor result for an
instantaneous Bianchi-I closure}
...
Gate: & \texttt{Analysis/Casimir/CBR-001/} \\
\end{tabular}%
}
...
For a free massless scalar on a rectangular flat three-torus we test an
already-renormalized periodic lattice formula for Casimir energy density and
directional pressures against cutoff convergence, cubic isotropy,
thermodynamic pressure derivatives, conformal trace, and dimensional scaling.
We then transport the static shape-dependent stress through a declared
instantaneous constitutive closure with \(a^{-4}\) dilution on a positive
de~Sitter biaxial Bianchi-I background.  In the bounded scan reported here,
this closure produces only \emph{transient} passages near the historically
proposed ratio \(\Ht/\Hp=13/12\), with no quasi-plateau or late-time
attractor.  This excludes a maintained ratio within the tested closure; it is
not a general no-go theorem for a quantum field evolved and renormalized on a
dynamical compact spacetime.
...
Here we isolate two controlled questions that can be addressed with the
declared lattice formula and biaxial closure integrator (gate package
CBR-001):

\begin{enumerate}
\item Does the implemented periodic-scalar stress pass its internal numerical
and thermodynamic controls and depend on the shape moduli?
\item When that static stress is transported by the declared instantaneous
closure on biaxial expansion, does
\(\Ht/\Hp=13/12\) appear as a free-field attractor, a quasi-plateau, or only
as a transient crossing (if at all)?
\end{enumerate}

The answers are: the implemented static stress passes those controls and is
shape dependent; and the bounded closure calculation yields only transient
crossings of \(13/12\), with every valid candidate returning to isotropy by
late expansion.
...
For a massless scalar with periodic lengths \((L_1,L_2,L_3)\), CBR-001 takes
as input the already-renormalized rectangular-periodic Epstein-lattice
expression developed in Ref.~\cite{edery2006}:
\begin{equation}
...
Extrapolated \(\rho\) uses a polynomial retained through \(1/N^2\), including
a fitted \(1/N\) term; validation residuals are at mode cutoff \(N=120\).}
\label{tab:stage1}
\begin{ruledtabular}
\begin{tabular}{lc}
Quantity & Value \\
...
Let \(r=L_t/L_p\).  CBR-001 Stage~2 holds \(L_p=1\) while varying
\(L_t=rL_p\); it does not hold the mean scale fixed.  Stage~3 converts this
raw table to fixed mean scale using the homogeneous \(r^{8/3}\) stress
rescaling before constructing its source.
...
\end{align}
where the second line is in the raw fixed-\(L_p\) Stage-2 convention.  Compact
\emph{shape-dependent} anisotropic stress is therefore a concrete control of
the implemented static formula.
...
% =============================================================================
\section{Biaxial instantaneous-stress closure}
...
It does not evolve quantum-field mode functions on the time-dependent
Bianchi-I geometry or renormalize \(\langle T_{\mu\nu}\rangle\) there.
The dimensionless source-scale ratio \(\epsilon\) absorbs
\(\hbar c/L_\star^4\) relative to the de~Sitter density scale (CBR-001
Stage~3A convention).  No present ITSM derivation fixes this ratio.
...
\(r_0\in\{1.01,1.05,1.1,1.25,1.5,2,3,4\}\) and source-scale ratios spanning seven decades
...
two no-crossings, and one trajectory initialized at the upper interpolation
boundary whose outward drive leaves the supported domain
(Table~\ref{tab:stage3b}; Fig.~\ref{fig:ratio}).
Dwell times within \(1\%\) of the target are only
\(\sim 0.27\)--\(0.30\) e-folds.
Every valid candidate returns to \(q\to 1\) by \(N=20\), consistent with the
declared \(a^{-4}\) source becoming negligible.  The tested nearby
trajectories also decay toward isotropy; none satisfy the registered attractor
criteria.
...
touch of \(q=13/12\).  Near-cubic shapes require large source-scale ratios;
...
\end{equation}
and, under the stated positive-\(H\), bounded-shape instantaneous closure,
cannot maintain \(\Pi_{\mathrm{req}}\) once the source redshifts away.
...
\item The renormalized static lattice expression is supplied as input; an
independent noncubic zeta/Ewald evaluation remains required.
\item The time-dependent calculation is an instantaneous \(a^{-4}\) closure,
not a mode-function evolution or curved-spacetime renormalization of
\(\langle T_{\mu\nu}\rangle\).  Particle production, curvature counterterms,
trace-anomaly and memory terms are not calculated.
...
When that static stress is transported as an instantaneous \(a^{-4}\) source
on biaxial de~Sitter expansion, the ratio \(\Ht/\Hp=13/12\) appears at most as
a brief transient at large source-scale ratios, never as a quasi-plateau or
attractor in the bounded search reported here.  The tested closure therefore
cannot sustain the historically proposed anisotropic expansion or justify a
map to
\(H_t=72.97\,\mathrm{km\,s^{-1}\,Mpc^{-1}}\).
Public code and stage summaries under \texttt{Analysis/Casimir/CBR-001/}
define the Stage-1--3B pipeline.  The current draft remains on publication
hold pending independent analytic and numerical audits.  Portable SHA-256
anchors for the repaired candidate are recorded in
\texttt{CBR001\_CHECKSUMS.md}; these anchors identify artifacts and do not
constitute a physics pass.
...
Table~\ref{tab:checksums} lists SHA-256 prefixes for portable CBR-001 CSV/JSON
artifacts underlying the Stage-1--3B numbers in the main text (branch
\texttt{recovery/v12-core-architecture}).  CSV writers use a fixed LF record
terminator and JSON output paths are relative to the summary directory.
...
\hline
\texttt{cbr001\_stage1.csv} & \texttt{57847F382BAF\ldots} \\
\texttt{cbr001\_stage2\_scan.csv} & \texttt{FE648629545F\ldots} \\
\texttt{cbr001\_stage3b\_thresholds.csv} & \texttt{94B0AE4C6381\ldots} \\
\texttt{cbr001\_stage3b\_summary.json} & \texttt{9A5E80D2F713\ldots} \\
\end{tabular}
\end{ruledtabular}
\end{table}

Full digests and reproduction commands appear in the companion
\texttt{CBR001\_CHECKSUMS.md} ledger.

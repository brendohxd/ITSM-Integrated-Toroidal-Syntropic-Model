...
\title{SPARC Galactic Kinematics under Conditional AQUAL/Picard Inputs\\{\large Quarantined legacy scaffold---not for citation or submission}}
...
\begin{abstract}
This file is a quarantined legacy scaffold retained for provenance and internal review. It does not report a validated ITSM fit to SPARC. In the current fail-closed authority, MAT-001 is blocked, DISK-001 is methods-only, and STAT-001 has not started as a closed gate. The first-principles coupling, zero-global-parameter, 175-galaxy and MCMC claims preserved in the body are not current results and must not be cited, submitted or archived as such.
\end{abstract}

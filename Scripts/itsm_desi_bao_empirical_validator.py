...
if os.path.exists(data_path):
    ...
    # 4. ITSM Empirical Validation Engine
    # Manifold: H(z) = H0 * sqrt(Om * (1+z)^3 + (1-Om) * (1+z)^-n_opt)
    H_itsm_calc = H0 * np.sqrt(Om * (1 + z_obs)**3 + (1 - Om) * (1 + z_obs)**-n_opt)
    chi2_nu = np.sum(((H_obs - H_itsm_calc) / err_H)**2) / len(z_obs)

    print(f"--- BAO EMPIRICAL VALIDATOR: DESI DR2 ---")
    ...
    plt.plot(z_smooth, H_smooth, '-', lw=3, label=rf'ITSM Optimized ($n={n_opt:.2f}$) [$\chi^2_\nu \approx {chi2_nu:.2f}$]')

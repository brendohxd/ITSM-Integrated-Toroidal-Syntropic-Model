...
# This version implements Alternative 1: Saturating Drag
# It provides the 1/sqrt(X) decay required for high-acceleration stability.
def get_itms_velocity(g_N, a0):
    # The First-Principles Shear Interaction
    # g_eff converges to g_N when g_N >> a0
    # g_eff converges to sqrt(g_N * a0) when g_N << a0
    ratio = g_N / a0
    shear_factor = (np.sqrt(1 + ratio**2) - 1) / (ratio + 1e-10)
    g_eff = g_N + (a0 * shear_factor)
    return np.sqrt(g_eff * data['Rad'])

data['V_ITSM'] = get_itms_velocity(g_N, a0_sparc)
